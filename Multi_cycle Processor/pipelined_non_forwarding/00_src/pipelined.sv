module pipelined (
    ...
    // ========================================================================
    // 0. BOOT STALL LOGIC (Quan trọng để fix lỗi lệnh đầu tiên)
    // ========================================================================
    logic is_booting;
    always_ff @(posedge i_clk) begin
        if (!i_reset) is_booting <= 1'b1; 
        else is_booting <= 1'b0;
    end
    ...
    // ========================================================================
    // 2. HAZARD UNIT (NON-FORWARDING VERSION)
    // ========================================================================
    // SỬ DỤNG: hazard_nonforwarding (Thay vì hazard_forwarding)
    hazard_nonforwarding  hazard_unit (
        .i_rs1_addr_id  (rs1_addr_id), 
        .i_rs2_addr_id  (rs2_addr_id),
        // .i_rs1_addr_ex  (rs1_addr_ex), // Không cần thiết cho logic stall
        ...
        // .i_is_load_ex   (wb_sel_ex == 2'b10), // Không cần phân biệt Load, mọi ghi đều Stall
        ...
        .o_rs1_sel      (fwd_a_sel), // Sẽ luôn trả về 00
        .o_rs2_sel      (fwd_b_sel)  // Sẽ luôn trả về 00
    );
    ...
    always_ff @(posedge i_clk) begin
        if (!i_reset) begin 
            pc_curr_id <= 0; pc_plus4_id <= 0; instr_id <= 0;
        end else if (flush_if_id) begin
            instr_id <= 0; pc_plus4_id <= 0; 
        end else if (!stall_if_id_final) begin
            pc_curr_id <= pc_curr_if; pc_plus4_id <= pc_plus4_if; instr_id <= instr_if;
        end
    end
    ...
    always_ff @(posedge i_clk) begin
        if (!i_reset) begin
            ...
            // Stall/Flush logic: Xóa sạch control và data
            reg_wren_ex <= 0; mem_wren_ex <= 0; wb_sel_ex <= 0; 
            alu_op_ex <= 0; op_a_sel_ex <= 0; op_b_sel_ex <= 0;
            rs1_data_ex <= 0; rs2_data_ex <= 0; imm_ex <= 0;
            ...
            pc_curr_ex <= pc_curr_id; pc_plus4_ex <= pc_plus4_id;
            ...
            ctrl_ex <= ctrl_id; mispred_ex <= mispred_id; valid_instr_ex <= valid_instr_id;
        end
    end
    ...
    // ========================================================================
    // Trong chế độ Non-forwarding, fwd_a_sel và fwd_b_sel LUÔN LÀ 00
    // (Được set cứng bởi hazard_nonforwarding)
    ...
    always_ff @(posedge i_clk) begin
        if (!i_reset) begin
            ...
            pc_curr_mem <= pc_curr_ex; pc_plus4_mem <= pc_plus4_ex; 
            alu_result_mem <= alu_result_ex; store_data_mem <= fwd_b_val; 
            rd_addr_mem <= rd_addr_ex; funct3_mem <= funct3_ex;
            ctrl_mem <= ctrl_ex; mispred_mem <= mispred_ex; valid_instr_mem <= valid_instr_ex;
        end
    end
    ...
    lsu LSU (
        .i_clk(i_clk), 
        .i_reset(~i_reset), // Giả sử Active Low Reset
    ...
    always_ff @(posedge i_clk) begin
        if (!i_reset) begin
            ...
            pc_curr_wb <= pc_curr_mem; pc_plus4_wb <= pc_plus4_mem; 
            alu_result_wb <= alu_result_mem; rd_addr_wb <= rd_addr_mem;
            ctrl_wb <= ctrl_mem; mispred_wb <= mispred_mem; valid_instr_wb <= valid_instr_mem;
        end
    end
