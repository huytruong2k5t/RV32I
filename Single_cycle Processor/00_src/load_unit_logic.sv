...
//
// Description   : Load unit logic performing byte/halfword alignment and sign/zero-extension.
//-----------------------------------------------------------------------------
// $Source: $
// $Revision: $
// $Log: $
...
module load_unit_logic (
    ...
    input  logic [6:0]  i_opcode,  // opcode LOAD
    output logic [31:0] o_ld_dataout  // sign/zero-extend
);

    //opcode LOAD (0000011 = 0x03)
    wire is_load;
    assign is_load = (~i_opcode[6]) & (~i_opcode[5]) & (~i_opcode[4]) &
                     (~i_opcode[3]) & (~i_opcode[2]) & (i_opcode[1]) & (i_opcode[0]);
